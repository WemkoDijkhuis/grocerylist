...
trait CorsHandler {
  ...
  private def preflightRequestHandler: Route = options {
    complete(HttpResponse(200).withHeaders(
      allowedOrigin :: `Access-Control-Allow-Methods`(OPTIONS, POST, PUT, GET, DELETE) :: List[HttpHeader]( `Access-Control-Allow-Headers`(corsAllowedHeaders.mkString(", ")))
    )
    )
  }

  val corsAllowedHeaders: List[String] = List("Authorization", "Origin", "X-Requested-With", "Content-Length", "Connection", "Content-Type", "Accept", "Accept-Encoding", "Accept-Language", "Host", "Referer", "User-Agent")
